...
> `Archiv/Lösung mit Produktionsstillstand`. Bot + Flow bereits dupliziert
...
| **System-Topics: Sicherheitsnetz bei Gesprächsabbruch** (NEU) | **erledigt (2026-09-02)** — **„Ende der Unterhaltung"** (`OnSystemRedirect`, `CancelOtherTopics`): Vor `EndConversation` neuer ConditionGroup-Node: Bedingung `Not(IsBlank(Global.Telefonnummer)) && Not(Global.FlowAufgerufen)` → `InvokeFlowAction` (flowId `019885f0-e29a-f111-b8db-7ced8d476627`) mit `If(IsBlank(...))` Wrappern für alle 6 Felder (Anliegen-Fallback: „Gespräch vorzeitig beendet – Anliegen nicht erfasst"; Anlage-Fallback: „nicht erfasst"). Feuert bei jedem Gesprächsende — auch bei Caller-Hang-Up via Teams-Phone-Signal. **„Stille-Erkennung"**: `EndDialog` → `BeginDialog → EndofConversation` ersetzt — stellt sicher, dass auch lautlose Abbrüche das Sicherheitsnetz durchlaufen. **Globale Variable `Global.FlowAufgerufen`** (Boolean): verhindert Doppel-Mail wenn Flow bereits regulär aufgerufen wurde. (YAML: `Archiv/Alte Yamls/Ende der Unterhaltung.txt`, `Archiv/Alte Yamls/Stille-Erkennung.txt`) **⚠ Bekannte Lücke (MCP-Review 2026-09-03):** Tritt der Fehler auf, bevor `Global.Telefonnummer` erfasst wurde (z. B. Laufzeitfehler während „Kundendaten erfassen"), greift das Sicherheitsnetz nicht — kein Flow-Aufruf, keine E-Mail, das Anliegen geht komplett verloren. Noch nicht behoben/entschieden. |
...
- [ ] **Englischer Anrufer — Zweisprachigkeit** (in Umsetzung seit 2026-09-08): Ersetzt den früheren Ansatz „eigenes Topic mit englischen Trigger-Phrasen". Umgesetzt wird stattdessen ein **Sprach-Fork am Gesprächsanfang** (Taste 1 / „English") mit `System.User.Language = en-US` und einer duplizierten englischen Slim-Kette. **Alles dazu — Entscheidungen, 9 Umsetzungsschritte, Übersetzungstabelle, Testprotokoll, Pflegeregeln — steht in [Konzept-Englisch.md](Konzept-Englisch.md).** Fortschritt dort im Testprotokoll abhaken.
  ...
  - [x] **Schritt 3 — `ConversationStart.mcs.yml` erweitert.** Beim Pull vorgefunden: bereits im Portal umgesetzt (Sprachfrage + `ConditionGroup` + `System.User.Language`/`Global.Sprache` per `SetMultipleVariables` + `BeginDialog` nach `KundendatenerfassenEN` bzw. `Kundendatenerfassen`) — funktional identisch zum Konzeptentwurf.
  - [x] **Schritt 4 — die fünf englischen Topics befüllt (2026-09-08).** Nutzer hatte 5 leere Platzhalter im Portal angelegt; nach Pull mit strukturgleichem, übersetztem Inhalt befüllt (`KundendatenerfassenEN`, `AnrufgrunderfassenEN`, `AnlageerfassenEN`, `AnliegenerfassenEN`, `ZusammenfassungEN`). Vier der fünf Platzhalter hatten `kind: OnRecognizedIntent` (Portal-Default) statt `OnRedirect` — korrigiert. **Neuer Fund:** `init:`-Präfix ist eine Pro-Variable-, keine Pro-Topic-Eigenschaft über den ganzen Agenten — `Global.Ansprechpartner`/`Global.Anliegen` waren durch die deutschen Topics bereits `init:`-deklariert, doppelte Deklaration in den EN-Topics gab `DuplicateVariableInitializer`; behoben durch Entfernen von `init:` in den EN-Pendants. Validiert: 32 Dateien, 0 Fehler, 0 Warnungen. **Offen:** die vom Nutzer im Portal vorgenommene Umbenennung `Zusammenfassung - Slim` → `Zusammenfassung` kam beim Pull **nicht** an (Datei/`componentName` unverändert) — im Portal gegenprüfen.
  - [x] **Zweiter Pull vor Schritt 5 (2026-09-08):** DE-Kette wurde in der Cloud zwischenzeitlich weiterentwickelt (Telefonnummer-Frage → `PhoneNumberPrebuiltEntity`, neue Variable `Global.TelefonGesprochen` für die Vorlese-Aussprache) — in `KundendatenerfassenEN` und `ZusammenfassungEN` identisch nachgezogen (Pflegeregel 1). Die 5 lokal befüllten EN-Topics blieben vom Pull unangetastet (nur `localChanges`, keine `remoteChanges`). Umbenennung `Zusammenfassung - Slim` → `Zusammenfassung` weiterhin nicht synchron.
  - [x] **Schritt 5 — Entities um englische Synonyme erweitert (2026-09-08).** `Anrufgrund.mcs.yml` und `Korrekturfeld.mcs.yml`: bestehende Entities erweitert, keine neuen `_EN`-Entities — `Global.Anrufgrund` ist eine einzige globale Variable beider Sprachzweige, deren Typ an den Options-Set der Entity gebunden ist; eine zweite Entity hätte einen inkompatiblen Typ erzeugt. `displayName` unangetastet (Flow-Vertrag). Validiert: 32 Dateien, 0 Fehler, 0 Warnungen.
  - [x] **Korrektur zu Schritt 5 (2026-09-08): der Ansatz „eine Entity für beide Sprachen" wurde wieder verworfen.** Die gemischtsprachigen Synonyme führten zu Fehlzuordnungen; stattdessen wurden zwei **eigene** Entities angelegt (Commit `34a43d5`) — der Satz „keine neuen `_EN`-Entities" oben gilt also **nicht mehr**:
    - **`entities/AnrufgrundEN.mcs.yml`** (`Anrufgrund EN`), 5 Items mit eigenen IDs: `8jiEHk` = malfunction, `KXrpcq` = maintenance, `yc12cg` = spare parts, `g6Wk6n` = callback, `tpthRC` = other. Dazu die **eigene** Variable `Global.AnrufgrundEN` — der englische Zweig füllt `Global.Anrufgrund` **nie**. Deshalb müssen alle sprachneutralen Stellen beide Variablen lesen (siehe `Ende der Unterhaltung`, `text_6`).
    - **`entities/JaNein.mcs.yml`** (`JaNein`).
    - Konsequenz: Der Agent hat jetzt 5 Entities — `Anrufgrund`, `AnrufgrundEN`, `JaNein`, `Korrekturfeld`, `Sprachwahl`.
  - [x] **Schritt 6 — Systemtopics zweisprachig machen (2026-09-09).** Alle fünf Dateien (`Fallback`, `EndofConversation`, `Silence`, `UnrecognizedSpeech`, `UnknownDtmfKey`) je um eine `ConditionGroup` auf `=Global.Sprache = "Englisch"` erweitert, deutscher Bestandsinhalt unverändert als `elseActions`. Details:
    - `Fallback.mcs.yml`: Englisch-Zweig redirectet auf `AnliegenerfassenEN` (statt `Anliegenerfassen`).
    - `EndofConversation.mcs.yml`: nur die `SendActivity` `BsLiWe` wurde zweisprachig; die sicherheitskritische `ConditionGroup` (`Not(IsBlank(Global.Telefonnummer)) && Not(Global.FlowAufgerufen)`) und die `InvokeFlowAction` blieben unangetastet.
    - `Silence.mcs.yml`: Der `Question`-Node wurde für Englisch **dupliziert statt geteilt** — eigene Variable `init:Topic.Var1EN` statt `Topic.Var1`, um ein mögliches `DuplicateVariableInitializer`-Risiko innerhalb desselben Topics zu vermeiden (vorsorglich, nicht am Schema verifiziert). Der abschließende `BeginDialog → EndofConversation` blieb unconditional, da `EndofConversation` selbst schon zweisprachig ist.
    - `UnknownDtmfKey.mcs.yml`: `activity:` blieb bewusst ein einfacher String (kein `text`/`speak`-Objekt), wie es der Node-Typ verlangt.
    - Alle englischen Formulierungen aus bereits etablierten Übersetzungen übernommen (`ZusammenfassungEN.mcs.yml`, Konzept-Übersetzungstabelle) — keine neuen Ad-hoc-Texte.
    - Validiert: 34 Dateien, 0 Fehler, 0 Warnungen. **Push in den Draft erfolgt (2026-09-09).**
    - **⚠ Zwischenfall beim Push:** Erster Push-Versuch scheiterte an `ConcurrencyVersionMismatch` (zwischenzeitlicher Publish-Zeitstempel in der Cloud, keine fremde inhaltliche Änderung). Der daraufhin ausgeführte `syncPull` hat `EndofConversation.mcs.yml` **stillschweigend auf den alten, einsprachigen Stand zurückgesetzt** — Bestätigung der Werkzeugfalle in CLAUDE.md, diesmal mit echtem Datenverlust-Risiko (Änderung war nie committet). Manuell aus dem Session-Kontext wiederhergestellt, per Fresh-Clone-Diff verifiziert (nur die 5 beabsichtigten Dateien wichen ab, keine fremden Inhalte), danach frisch gepullt + erneut erfolgreich gepusht. **Lehre:** Nach jeder inhaltlichen Topic-Änderung zeitnah committen — unkommittete Änderungen sind bei einem Pull-Konflikt nicht durch Git absicherbar.
...
- [ ] **Freitextfelder werden ungeprüft in die E-Mail interpoliert** (notiert
      ...
      `Archiv/Email Body.html`, z. B. Zeile 39) sowie im Prompt des KI-Nodes.
...
- [x] **#18 — Keine E-Mail bei abgebrochener Wartungsanfrage**: durch denselben
  ...
  **Nachbessserung (2026-09-02, Kundentermin-Feedback):** Lücke geschlossen: Wenn
  ...
  Verifiziert per Datei-Review 2026-09-03 (`Archiv/agent/AIRCO Telefon-Bot/topics/OnError.mcs.yml`,
  vormals `Archiv/Alte Yamls/Bei Fehler.md` — Datei entfernt, da veraltet gegenüber dem
...
- [ ] **#17 — „Themenerkennung": Bot sagt „kein Thema verknüpft" bei fachfremden Aussagen**:
  ...
  **Entschieden (2026-08-25)**: „Bei Fehler" soll das Gespräch nach der
  ...
  (`LogCustomTelemetryEvent`) bleibt erhalten. (YAML: `Archiv/agent/AIRCO Telefon-Bot/topics/OnError.mcs.yml`) ✅
...
- [x] **ERLEDIGT (2026-09-08) — Der Ordner `YAML/` war eine zweite, driftende
  Quelle.** `Archiv/Alte Yamls/Kundendaten_erfassen.md` kannte die Anrufgrund-Frage und den
  Staging-Aufruf nicht; `Archiv/Alte Yamls/Anrufgrund erfassen.md` enthielt noch die Frage,
  die längst in `Kundendaten erfassen` sitzt. Seit der Agent per
  `manage-agent pull` nach `Archiv/agent/AIRCO Telefon-Bot/` gespiegelt wird, ist das
  ...
  **`Archiv/Alte Yamls/`** und sind ausdrücklich *historischer Arbeitsstand, keine
  Referenz*. `Agent Definition - KI-Verarbeitung.md` und `Archiv/Email Body.html`
  liegen im Repo-Root, weil sie **kein** Backup, sondern weiterhin aktuelles
  Referenzmaterial für den Flow sind.
  **Einzige Quelle der Wahrheit für Topics: `Archiv/agent/AIRCO Telefon-Bot/`.**
...
- [x] E-Mail-Template `Archiv/Email Body.html` aktualisiert und im Flow eingesetzt:
...
`Archiv/agent/AIRCO Telefon-Bot/topics/Kundendatenerfassen.mcs.yml`. Die beiden
